...
void LoadActor(const rapidjson::Value& actorValue, Game* game, Actor* parent)
{
	if (actorValue.IsObject())
	{
		...
		if (type == "Block")
		{
			...
			actor = block;
			bool reflect = false;
			if (GetBoolFromJSON(actorValue, "mirror", reflect))
			{
				block->SetReflect(reflect);
			}
			bool rotatable = false;
			if (GetBoolFromJSON(actorValue, "rotates", rotatable))
			{
				block->SetRotate(rotatable);
			}
		...
		else if (type == "SecurityCamera")
		{
			...
			Quaternion startQ;
			if (GetQuaternionFromJSON(actorValue, "startQ", startQ)) sc->startQ = startQ;
			Quaternion endQ;
			if (GetQuaternionFromJSON(actorValue, "endQ", endQ)) sc->endQ = endQ;
			float interpTime;
			if (GetFloatFromJSON(actorValue, "interpTime", interpTime)) sc->interpTime = interpTime;
			float pauseTime;
			if (GetFloatFromJSON(actorValue, "pauseTime", pauseTime)) sc->pauseTime = pauseTime;

			sc->SetRotationQ(startQ);
		}
